...
class AntiWebhookUpdate(commands.Cog):
    ...
    async def fetch_audit_logs(self, guild, action):
        ...
        Webhook audit log targets are the webhook objects, not the channel,
        so we can't filter by channel.id — we just take the most recent recent entry.
        """
    ...
    async def on_webhooks_update(self, channel):
        ...
        entry = await self.fetch_audit_logs(guild, discord.AuditLogAction.webhook_update)
        if entry is None:
            return

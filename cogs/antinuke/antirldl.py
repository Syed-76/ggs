...
class AntiRoleDelete(commands.Cog):
        ...
        try:
            ...
            async for entry in guild.audit_logs(action=action, limit=5):
                ...
                if difference >= 3600000:
                    break  # entries are newest-first; if this is old, rest will be older
    ...
    async def on_guild_role_delete(self, role):
        ...
        log_entry = await self.fetch_audit_logs(guild, discord.AuditLogAction.role_delete, role.id)
    ...
    async def ban_executor(self, guild, executor):
        ...
        while retries > 0:
            try:
                ...
                try:
                    from utils.log_bridge import send_antinuke_log
                    await send_antinuke_log(
                        self.bot, guild,
                        title="Anti-Role-Delete Triggered",
                        description=(
                            f"**{executor}** (`{executor.id}`) was **banned** for "
                            f"deleting a role. Role was recreated."
                        ),
                        executor=executor,
                    )
                except Exception:

...
class AntiSpam(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if user_messages:
            ...
            self.recent_messages.pop(user.id, None)
        ...
        if len(user_messages) > self.spam_threshold:
            ...
            try:
                ...
                simple_embed.description = f"{_e('ztick')} | {user.mention} has been successfully **{action_taken}** for **Spamming.**"
                ...
                try:
                    from utils.log_bridge import send_automod_log
                    await send_automod_log(
                        self.bot, guild, rule="Anti-Spam",
                        user=user, channel=channel,
                        action=action_taken, reason=reason,
                    )
                except Exception:
                    pass

...
class General(commands.Cog):
  ...
  async def _user(self, ctx, member: Optional[Union[discord.Member, discord.User]] = None):
    try:
      ...
      embed.set_author(name=f"{member}", icon_url=member.display_avatar.url if member.avatar else member.default_avatar.url)
  ...
  # NOTE: the old reaction-based `poll` command was replaced by the full
  # interactive poll builder in cogs/commands/poll.py (PollCog). See there
  # for the `/poll` and `>poll` implementation.
  ...
  @commands.command(name="users", help="checks total users of this bot.")
  @blacklist_check()
  @ignore_check()
  @commands.cooldown(1, 3, commands.BucketType.user)
  async def users(self, ctx: commands.Context):
    ...
    embed = discord.Embed(
      title=f"**{self.bot.user.name} Users**",
  ...
  async def invite(self, ctx: commands.Context):
      embed = discord.Embed(
          ...
          description="```Empower your server with blazing-fast features!```",
          color=0xFF0000
      )
      embed.set_thumbnail(url=self.bot.user.display_avatar.url)
      embed.add_field(
          name="🔗 Invite",
          value=f">>> **[Invite {self.bot.user.name}](https://discord.com/oauth2/authorize?client_id={self.bot.user.id}&permissions=8&integration_type=0&scope=bot+applications.commands)**",
          inline=False
      )
      embed.set_footer(

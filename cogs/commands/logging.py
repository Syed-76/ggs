...
class LogSetupView (View ):
    ...
    @discord .ui .button (label ="Finish Setup",style =discord .ButtonStyle .success ,row =0 )
    async def finish_button (self ,interaction :discord .Interaction ,button :Button ):
        """Finish the setup process."""
        await self .finish_setup (interaction )

    @discord .ui .button (label ="⚡ Auto Setup",style =discord .ButtonStyle .primary ,row =1 )
    async def auto_setup_button (self ,interaction :discord .Interaction ,button :Button ):
        """Auto-create a log category with all 9 log channels pre-configured."""
        view =AutoLogSetupView (self .bot ,self .author )
        embed =discord .Embed (
            title ="⚡ Auto Log Setup",
            description =(
                "This will automatically create a **log category** and **9 channels** for all log types.\n\n"
                "Optionally select a **staff role** to grant read access to log channels.\n"
                "All channels will deny **@everyone** by default. "
                "Note: Discord server administrators always bypass channel permissions regardless of this setting."
            ),
            color =0xFF0000 ,
            timestamp =datetime .utcnow (),
        )
...
class AutoLogSetupView (View ):
    """Role-select + confirm view for the ⚡ Auto Setup flow."""

    _CATEGORY_CHANNELS ={
        "message_events":"msg-logs",
        "join_leave_events":"join-leave-logs",
        "member_moderation":"mod-logs",
        "voice_events":"voice-logs",
        "channel_events":"channel-logs",
        "role_events":"role-logs",
        "emoji_events":"emoji-logs",
        "reaction_events":"reaction-logs",
    ...
    @discord .ui .select (
        ...
        placeholder ="(Optional) Select staff role for log channels…",
        min_values =0 ,
        max_values =1 ,
        row =0 ,
    )
    async def role_select (self ,interaction :discord .Interaction ,select :discord .ui .RoleSelect ):
        self .staff_role =select .values [0 ] if select .values else None 
        await interaction .response .defer ()

    @discord .ui .button (label ="✅ Create Log Channels",style =discord .ButtonStyle .success ,row =1 )
    async def create_button (self ,interaction :discord .Interaction ,button :Button ):
        ...
        try :
            ...
            category_name =f"{branding_name} logs"

            # Find a visible text channel to duplicate so cloned channels show to all users by default
            source_channel =next (
                (ch for ch in interaction .guild .text_channels
                 if ch .overwrites .get (interaction .guild .default_role ,discord .PermissionOverwrite ()).read_messages is not False ),
                interaction .guild .text_channels [0 ]if interaction .guild .text_channels else None 
            )

            # Category has no @everyone restriction → all channels visible by default
            overwrites ={
                interaction .guild .me :discord .PermissionOverwrite (read_messages =True ,send_messages =True ,embed_links =True ),
            }
            if self .staff_role :
                overwrites [self .staff_role ]=discord .PermissionOverwrite (read_messages =True ,send_messages =False )

            category =await interaction .guild .create_category (category_name ,overwrites =overwrites )

            selected_channels ={}
            for cat ,ch_name in self ._CATEGORY_CHANNELS .items ():
                if source_channel :
                    # Duplicate an existing visible channel — inherits its visible-to-everyone permissions
                    ch =await source_channel .clone (name =ch_name )
                    await ch .edit (category =category ,sync_permissions =True )
                else :
                    ch =await category .create_text_channel (ch_name )
                selected_channels [cat ]=ch .id 
            ...
            if cog :
                await cog ._save_log_config (
                    interaction .guild .id ,selected_channels ,log_enabled ,[],[],[],None ,
                    category .id
                )

            lines =[f"✅ **#{self._CATEGORY_CHANNELS[cat]}** → <#{selected_channels[cat]}>" for cat in LOG_CATEGORIES ]
            embed =discord .Embed (
                title ="✅ Auto Log Setup Complete",
                description =(
                    f"Created category **{category_name}** with **{len(LOG_CATEGORIES)}** channels.\n\n"
                    +"\n".join (lines )
                ),
                color =0x00FF00 ,
                timestamp =datetime .utcnow (),
            ...
            if self .staff_role :
                embed .add_field (name ="Staff Role",value =self .staff_role .mention ,inline =False )
            embed .set_footer (text ="All logging categories are now active. Use /log status to review." )
            ...
            await interaction .followup .send ("❌ Missing permissions to create channels/categories.",ephemeral =True )
    ...
    async def cancel_button (self ,interaction :discord .Interaction ,button :Button ):
        for item in self .children :
            item .disabled =True 
        embed =discord .Embed (description ="Auto setup cancelled.",color =0xFF0000 )
        await interaction .response .edit_message (embed =embed ,view =self )
...
class Logging (commands .Cog ):
    ...
    async def _save_config (self ):
        ...
        async with self .config_lock :
            try :
                ...
                # Ensure the jsondb directory exists before writing
                await asyncio .to_thread (os .makedirs ,os .path .dirname (CONFIG_FILE ),exist_ok =True )
    ...
    auto_log_category_id :Optional [int ] =None
    ):
        ...
        try :
            ...
            "auto_log_category_id":auto_log_category_id if isinstance (auto_log_category_id ,int )else None ,
    ...
    async def log_reset (self ,ctx :commands .Context ):
        ...
        try :
            config =await self ._get_log_config (ctx .guild .id )
            if config :
                # Reset means removing the channels created/configured by the
                # logging system, not only forgetting their IDs in JSON.
                channel_ids =set (
                    config .get ("log_channels",{}).values ()
                    if isinstance (config .get ("log_channels",{}),dict )
                    else []
                )
                configured_channels =[
                ...
                for channel in configured_channels :
                    if channel and getattr (channel ,"category_id",None ):
                        category_ids .add (channel.category_id )
                    if channel :
                        try :
                            await channel .delete (reason ="Logging configuration reset")
                        except (discord .NotFound ,discord .Forbidden ,discord .HTTPException ):
                            pass

                stored_category_id =config .get ("auto_log_category_id")
                if isinstance (stored_category_id ,int ):
                    category_ids .add (stored_category_id )

                for category_id in category_ids :
                    category =ctx .guild .get_channel (category_id )
                    if category and isinstance (category ,discord.CategoryChannel ):
                        try :
                            await category .delete (reason ="Logging configuration reset")
                        except (discord .NotFound ,discord .Forbidden ,discord .HTTPException ):
                            pass
    ...
    async def on_member_remove (self ,member :discord .Member ):
        """Log member leaves and detect kicks via audit log."""
        ...
        try :
            ...
            if kick_entry :
                kick_embed =self ._create_modern_embed (f"{_e('zsowrd')} Member Kicked")
                kick_embed .set_author (name =member .display_name ,icon_url =member .display_avatar .url )
                kick_embed .add_field (name ="Member",value =f"{member.mention} ({member})",inline =True )
                kick_embed .add_field (name ="Kicked by",value =kick_entry .user .mention ,inline =True )
                if kick_entry .reason :
                    kick_embed .add_field (name ="Reason",value =kick_entry .reason [:1024 ],inline =False )
                kick_embed .set_footer (text =f"User ID: {member.id}")
                kick_embed .set_thumbnail (url =member .display_avatar .url )
                await self ._send_log (member .guild ,"member_moderation",kick_embed ,None ,member .id )

            # ── Always log join/leave ─────────────────────────────────────────
    ...
    async def on_member_update (self ,before :discord .Member ,after :discord .Member ):
        """Log member updates: role changes → role_events, timeout → member_moderation."""
        from utils .emojis import e as _e
        try :
            # ── Role changes → role_events (with who gave/removed) ───────────
            if before .roles !=after .roles :
                added_roles =set (after .roles )-set (before .roles )
                removed_roles =set (before .roles )-set (after .roles )

                # Fetch audit log to find who changed the roles
                audit_entry =None 
                try :
                ...
                if added_roles :
                    ...
                    embed .add_field (name =f"{_e('zplus')} Roles Added",value =roles_added ,inline =False )

                if removed_roles :
                    roles_removed =", ".join ([role .mention for role in removed_roles ])
                    embed .add_field (name =f"{_e('zcross')} Roles Removed",value =roles_removed ,inline =False )

                embed .set_footer (text =f"User ID: {after.id}")
                await self ._send_log (after .guild ,"role_events",embed ,None ,after .id )

            # ── Timeout → member_moderation ──────────────────────────────────
            ...
            if before_timeout !=after_timeout :
                ...
                if after_timeout :
                    embed =self ._create_modern_embed (f"{_e('ztimer')} Member Timed Out")
                    embed .set_author (name =after .display_name ,icon_url =after .display_avatar .url )
                    embed .add_field (name ="Member",value =after .mention ,inline =True )
                    embed .add_field (name ="Until",value =discord .utils .format_dt (after_timeout ,'F'),inline =True )
                    if audit_entry :
                        embed .add_field (name ="Timed out by",value =audit_entry .user .mention ,inline =True )
                        if audit_entry .reason :
                            embed .add_field (name ="Reason",value =audit_entry .reason [:1024 ],inline =False )
                else :
                    embed =self ._create_modern_embed (f"{_e('ztick')} Timeout Removed")
                    embed .set_author (name =after .display_name ,icon_url =after .display_avatar .url )
                    embed .add_field (name ="Member",value =after .mention ,inline =True )
                    if audit_entry :
                        embed .add_field (name ="Removed by",value =audit_entry .user .mention ,inline =True )

                embed .set_footer (text =f"User ID: {after.id}")
                await self ._send_log (after .guild ,"member_moderation",embed ,None ,after .id )

            # ── Nickname change → member_moderation ──────────────────────────

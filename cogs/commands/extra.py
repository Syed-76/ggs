...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed.add_field(name="__UTC__", value=f"⚠️ {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"⏱️ {uptime_duration_string}", inline=False)
  ...
  async def _userinfo(self,
    ...
    embed.set_author(name=f"{member.name}'s Information",
                     ...
                     if member.avatar else member.default_avatar.url)
    embed.set_thumbnail(
      url=member.display_avatar.url if member.avatar else member.default_avatar.url)
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def ping(self, ctx: commands.Context):
    ...
    final_embed.set_footer(text=f"Requested by {ctx.author.name}", icon_url=ctx.author.display_avatar.url if ctx.author.avatar else None)

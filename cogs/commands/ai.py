...
class PersonalityModal (discord .ui .Modal ,title ="Set Your AI Personality"):
    def __init__ (self ,ai_cog ,current_personality :str =""):
        ...
        default_prompt =f"""You are {DEFAULT_BRANDING}, an intelligent and caring Discord bot assistant. 💕
...
Ready to have meaningful conversations and help with anything you need! 💖"""
    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        ...
        try :
            ...
            embed =discord .Embed (
            title =f"{_e('starr')} Personality Set",
    ...
    async def _get_response (self ,message :str ,history :list ,guild_id :int ,user_id :int =None )->str :
        try :
            ...
            if user_personality :
                ...
                system_context .append ({
                "role":"system",
                "content":"You are a Discord bot with many features including moderation, entertainment, music, games, AI capabilities, and utilities. Support server: https://discord.gg/TW9dTu7YKS"
                })
            else :
...
Support server: https://discord.gg/TW9dTu7YKS"""
                })
        ...
        if ctx .message .reference and ctx .message .reference .message_id :
            try :
                ...
                if replied_message .attachments :
                    ...
                    embed =discord .Embed (
                    title =f"{_e('zyrox_search')} Analysis",
        ...
        if not image_url :
            ...
            async for message in ctx .channel .history (limit =20 ):
                ...
                if ctx .message .content and ctx .message .content .strip ():
                    ...
                    embed =discord .Embed (
                    title =f"{_e('zimage')} Image Analysis / {_e('zmsg')} Text Analysis",
    ...
    async def analyze_image (self ,ctx ,image_url :str ):
        ...
        try :
            if not self .gemini_api_key :
                ...
                title =f"{_e('zimage')} Image Analysis",
    ...
    async def ai_code (self ,ctx :commands .Context ,language :str ,*,description :str ):
        ...
        try :
            ...
            if len (formatted_code )<=3900 :
                embed =discord .Embed (
                title =f"{_e('codebase')} Generated Code",
    ...
    async def ai_explain (self ,ctx :commands .Context ,*,topic :str ,level :str ="intermediate"):
        ...
        try :
            ...
            embed =discord .Embed (
            title =f"{_e('zbook')} Explanation: {topic}",
    ...
    async def ai_conversation_clear (self ,ctx :commands .Context ):
        ...
        embed =discord .Embed (
        title =f"{_e('zbroom')} Conversation Cleared",
    ...
    async def ai_mood_analyzer (self ,ctx :commands .Context ,*,text :str ):
        ...
        try :
            ...
            embed =discord .Embed (
            title =f"{_e('zmood')} Mood Analysis",
    ...
    async def ai_personality (self ,ctx :commands .Context ):
        ...
        if not hasattr (ctx ,'interaction')or not ctx .interaction :
            embed =discord .Embed (
            title =f"{_e('zmask')} AI Personality",
    ...
    async def ai_conversation_stats (self ,ctx :commands .Context ):
        ...
        if not stats :
            embed =discord .Embed (
            title =f"{_e('zpoll')} Conversation Statistics",
            ...
            embed =discord .Embed (
            title =f"{_e('zpoll')} Your Conversation Statistics",
            description =f"Here's our chat history!",
            color =0xFF0000 ,
            timestamp =datetime .now (timezone .utc )
            )
            embed .add_field (name =f"{_e('zmsg')} Total Messages",value =f"{stats['message_count']} messages",inline =True )
            embed .add_field (name =f"{_e('ztimer')} First Chat",value =f"<t:{int(first_msg.timestamp())}:R>",inline =True )
            embed .add_field (name =f"{_e('ztimer')} Last Chat",value =f"<t:{int(last_msg.timestamp())}:R>",inline =True )
            embed .add_field (
            name =f"{_e('zflower')} Memory Info",
    ...
    async def ai_activate (self ,ctx :commands .Context ,channel :discord .TextChannel =None ):
        ...
        if not ctx .author .guild_permissions .manage_channels :
            embed =discord .Embed (
            title =f"{_e('zcross')} Permission Denied",
        ...
        embed =discord .Embed (
        title =f"{_e('ztick')} AI Chatbot Activated",
    ...
    async def ai_deactivate (self ,ctx :commands .Context ):
        ...
        embed =discord .Embed (
        title =f"{_e('zunmute')} AI Chatbot Deactivated",
    ...
    async def ai_summarize (self ,ctx :commands .Context ,*,text :str ):
        ...
        if not text :
            embed =discord .Embed (
            title =f"{_e('zmsg')} Text Summarizer",
    ...
    async def ai_database_clear (self ,ctx :commands .Context ):
        ...
        try :
            ...
            embed =discord .Embed (
            title =f"{_e('delete')} Your Data Cleared",
    ...
    async def handle_trivia_answer (self ,interaction :discord .Interaction ,channel_id :int ,selected_answer :str ):
        ...
        response_message =(
        f"{_e('ztada')} First to answer! Correct! The answer was **{game['current_answer']}**. +1 point!"
        if is_correct 
        else f"{_e('zcross')} Incorrect. The answer was **{game['current_answer']}**. Your guess: **{selected_answer}**. -1 point."
        )
    ...
    async def show_stats (self ,ctx ):
        ...
        if not stats :
            embed =discord .Embed (
            title =f"{_e('zbrain')} Trivia Statistics",
        ...
        embed =discord .Embed (
        title =f"{_e('zbrain')} Your Trivia Statistics",
    ...
    async def show_leaderboard (self ,ctx ):
        ...
        if not top_scores :
            embed =discord .Embed (
            title =f"{_e('king')} Trivia Leaderboard",
    ...
    async def enable_roleplay (self ,ctx ):
        ...
        if channel_id in self .roleplay_channels :
            embed =discord .Embed (
            title =f"{_e('zmask')} Roleplay Mode",

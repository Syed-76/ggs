...
class Whitelist(commands.Cog):
    ...
    async def whitelist(self, ctx, member: discord.Member = None):
        if ctx.guild.member_count < 2:
            embed = discord.Embed(
                color=0xFF0000,
                description=f"{_e('zcross')} | Your Server Doesn't Meet My 30 Member Criteria"
            )
        ...
        if not is_owner and not check:
            embed = discord.Embed(title=f"{_e('zcross')} Access Denied",
                color=0xFF0000,
        ...
        if not member:
            ...
            embed.add_field(name="__**Usage**__", value=f"➡️ `{prefix}whitelist @user/id`\n➡️ `{prefix}wl @user`")
            embed.set_thumbnail(url=ctx.bot.user.display_avatar.url)
            return await ctx.send(embed=embed)
        ...
        embed.set_footer(text=f"Developed by {DEFAULT_BRANDING} Development™")

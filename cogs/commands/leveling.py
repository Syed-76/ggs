...
class PlaceholdersView (discord .ui .View ):
    ...
    @discord .ui .button (label ="Show Placeholders",style =discord .ButtonStyle .secondary ,emoji =__import__('utils.emojis',fromlist=['e']).e("zclipboard") or "📋")
    async def show_placeholders (self ,interaction :discord .Interaction ,button :discord .ui .Button ):
        from utils.emojis import e as _e
        embed =discord .Embed (
        title =f"{_e('zmsg')} Available Placeholders",
    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        try :
            ...
            try :
                ...
                await interaction .followup .send (f"{_e('zcross')} Invalid XP per message value! Must be between 1 and 999.",ephemeral =True )
                return 
            ...
            if not validate_hex_color (color_value ):
                await interaction .followup .send (f"{_e('zcross')} Invalid hex color format! Use format like #FF0000",ephemeral =True )
                return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} Leveling Configuration Updated",
            ...
            f"{_e('zcross')} An unexpected error occurred: {str(e)}",
            ephemeral =True 
            )
...
class Leveling (commands .Cog ):
    ...
    async def on_message (self ,message ):
        ...
        try :
            ...
            min_xp_setting =settings .get ('min_xp',None )
            max_xp_setting =settings .get ('max_xp',None )
            if min_xp_setting is not None and max_xp_setting is not None and max_xp_setting >=min_xp_setting :
                base_xp =random .randint (min_xp_setting ,max_xp_setting )
            else :
...
└─────────────────────────────────────────┘
    ...
    async def interactive_setup (self ,ctx ,current_settings ):
        ...
        try :
            embed =discord .Embed (
            title =f"{_e('zwrench')} Interactive Leveling Setup",
            ...
            value =f"{_e('zwrench')} Status: {'enabled' if current_settings.get('enabled', False) else 'disabled'}\n"
            f"{_e('starr')} XP per Message: {current_settings.get('xp_per_message', 20)}\n"
            f"{_e('zsettings')} Embed Color: {current_settings.get('embed_color', '#000000')}\n"
            f"{_e('zimage')} Thumbnail: {'yes' if current_settings.get('thumbnail_enabled', True) else 'no'}\n"
            f"{_e('zcast')} Level Channel: {'Set' if current_settings.get('channel_id') else 'Not set'}",
            inline =False 
            )
            ...
            await ctx .send (f"{_e('starr')} **How much XP should users get per message?** (1-999)\n"
            ...
            try :
                ...
                if xp_msg .content .lower ()=='cancel':
                    await ctx .send (f"{_e('zcross')} Setup cancelled.")
                    return 
                ...
                if xp_msg .content .lower ()=='skip':
                    ...
                    if xp_per_message <1 or xp_per_message >999 :
                        await ctx .send (f"{_e('zcross')} XP must be between 1 and 999. Using current value.")
                        xp_per_message =current_settings .get ('xp_per_message',20 )
            except (ValueError ,asyncio .TimeoutError ):
                await ctx .send (f"{_e('ztimer')} Timeout or invalid input. Using current value.")
                xp_per_message =current_settings .get ('xp_per_message',20 )


            current_msg =current_settings .get ('level_message','Congratulations {user}! You have reached level {level}!')
            await ctx .send (f"{_e('zmsg')} **What should the level-up message say?**\n"
            ...
            try :
                ...
                if msg_response .content .lower ()=='skip':
                    ...
                    if len (level_message )>2000 :
                        await ctx .send (f"{_e('zcross')} Message too long (max 2000 chars). Using current value.")
                        level_message =current_msg 
            except asyncio .TimeoutError :
                await ctx .send (f"{_e('ztimer')} Timeout. Using current value.")
                level_message =current_msg 


            current_color =current_settings .get ('embed_color','#000000')
            await ctx .send (f"{_e('zsettings')} **What color should the level-up embeds be?** (hex format like #FF0000)\n"
            ...
            try :
                ...
                if color_msg .content .lower ()=='skip':
                    ...
                    if validate_hex_color (color_input ):
                        ...
                        await ctx .send (f"{_e('zcross')} Invalid hex color. Using current value.")
                        embed_color =current_color 
            except asyncio .TimeoutError :
                await ctx .send (f"{_e('ztimer')} Timeout. Using current value.")
                embed_color =current_color 
            ...
            await ctx .send (f"{_e('zimage')} **Level-up image URL** (optional, direct image link)\n"
            ...
            try :
                ...
                if image_msg .content .lower ()=='skip':
                    ...
                    if level_image and len (level_image )>500 :
                        await ctx .send (f"{_e('zcross')} URL too long. Using current value.")
                        level_image =current_image 
            except asyncio .TimeoutError :
                await ctx .send (f"{_e('ztimer')} Timeout. Using current value.")
                level_image =current_image 
            ...
            await ctx .send (f"{_e('zimage')} **Show user avatar thumbnail in level-up messages?** (yes/no)\n"
            ...
            title =f"{_e('ztick')} Leveling System Configured!",
            ...
            name =f"{_e('zpoll')} Configuration Summary",
            value =f"{_e('zwrench')} **Status:** Enabled\n"
            f"{_e('starr')} **XP per Message:** {xp_per_message}\n"
            f"{_e('zmsg')} **Level-up Message:** {level_message[:50]}{'...' if len(level_message) > 50 else ''}\n"
            f"{_e('zsettings')} **Embed Color:** {embed_color}\n"
            f"{_e('zimage')} **Level-up Image:** {'Set' if level_image_final else 'None'}\n"
            f"{_e('zimage')} **Show Thumbnail:** {'Yes' if thumbnail_enabled else 'No'}",
            inline =False 
            )

            final_embed .add_field (
            name =f"{_e('zrocket')} Next Steps",
            value =f"• Use `{ctx.prefix}level channel #channel` to set announcement channel\n"
            f"• Use `{ctx.prefix}level rewards add <level> @role` to add level rewards\n"
            f"• Use `{ctx.prefix}level leaderboard` to view the server leaderboard\n"
    ...
    async def setxp (self ,ctx ,amount :int ):
        ...
        try :
            if amount <1 or amount >999 :
                await ctx .send (f"{_e('zcross')} XP per message must be between 1 and 999!")
                return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} XP Amount Updated",
    ...
    async def setmessage (self ,ctx ,*,message :str ):
        ...
        try :
            if len (message )>2000 :
                await ctx .send (f"{_e('zcross')} Level-up message must be less than 2000 characters!")
                return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} Level-Up Message Updated",
    ...
    async def setcolor (self ,ctx ,color :str ):
        ...
        try :
            ...
            if not validate_hex_color (color ):
                await ctx .send (f"{_e('zcross')} Invalid hex color format! Use format like #FF0000")
                return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} Embed Color Updated",
    ...
    async def thumbnail (self ,ctx ,setting :str ):
        ...
        try :
            ...
            if setting not in ['on','off','true','false','yes','no','enable','disable']:
                await ctx .send (f"{_e('zcross')} Use: `on/off`, `true/false`, `yes/no`, or `enable/disable`")
                return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} Thumbnail Setting Updated",
    ...
    async def cooldown (self ,ctx ,seconds :int ):
        ...
        try :
            if seconds <0 or seconds >3600 :
                await ctx .send (f"{_e('zcross')} Cooldown must be between 0 and 3600 seconds (1 hour)!")
                return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} Cooldown Updated",
    ...
    async def level_leaderboard (self ,ctx ):
        ...
        try :
            ...
            if not top_users :
                embed =discord .Embed (
                title =f"{_e('zpoll')} Server Level Leaderboard",
    ...
    async def placeholders (self ,ctx ):
        ...
        embed =discord .Embed (
        title =f"{_e('zclipboard')} Available Placeholders",
        description =(
    ...
    async def enable (self ,ctx ):
        try :
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if result :
                    if result [0 ]==1 :
                        await ctx .send (f"{_e('zwarning')} Leveling system is already enabled!")
                        return 
            ...
            embed =discord .Embed (
            title =f"{_e('ztick')} Leveling System Enabled",
            description ="The leveling system has been successfully enabled for this server!\n\n"
            "**Next Steps:**\n"
            f"• Use `{ctx.prefix}level settings` to configure XP rates and messages\n"
            f"• Use `{ctx.prefix}level channel` to set level-up announcement channel\n"
            f"• Use `{ctx.prefix}level rewards add` to set up level rewards",
            color =0x00FF00 ,
            timestamp =datetime .now (timezone .utc )
            )
            await ctx .send (embed =embed )
    ...
    async def disable (self ,ctx ):
        try :
            async with aiosqlite .connect (self .db_path )as db :
                ...
                if not result :
                    await ctx .send (f"{_e('zwarning')} Leveling system is not configured for this server!")
                    return 

                if result [0 ]==0 :
                    await ctx .send (f"{_e('zwarning')} Leveling system is already disabled!")
                    return 
            ...
            embed =discord .Embed (
            title =f"{_e('zcross')} Leveling System Disabled",
    ...
    async def stats (self ,ctx ,member :Optional [discord .Member ]=None ):
        ...
        try :
            ...
            embed =discord .Embed (
            title =f"{_e('zpoll')} Level Statistics for {member.display_name}",
            color =0xFF0000 ,
            timestamp =datetime .now (timezone .utc )
            )
            ...
            embed .add_field (
            name =f"{_e('zyroxthunder')} Basic Stats",
            value =f"**Level:** {level:,}\n"
            ...
            embed .add_field (
            name =f"{_e('zlevelup')} Progress to Level {{next_level}}",
            ...
            embed .add_field (
            name =f"{_e('zpoll')} Additional Stats",
            ...
            if rank ==1 :
                tier =f"{_e('king')} Champion"
                tier_color =0xFFD700 
            elif rank <=3 :
                tier =f"{_e('starr')} Elite"
                tier_color =0xC0C0C0 
            elif rank <=10 :
                tier =f"{_e('starr')} Expert"
                tier_color =0xCD7F32 
            elif percentile >=90 :
                tier =f"{_e('zlevelup')} Advanced"
                tier_color =0x00FFFF 
            elif percentile >=70 :
                tier =f"{_e('zyroxthunder')} Experienced"
                tier_color =0xFF4500 
            elif percentile >=50 :
                tier =f"{_e('zyroxthunder')} Intermediate"
                tier_color =0xFFFF00 
            elif percentile >=25 :
                tier =f"{_e('starr')} Beginner"
                tier_color =0x90EE90 
            else :
                tier =f"{_e('zseed')} Newcomer"
                tier_color =0x98FB98 

            embed .add_field (
            name =f"{_e('king')} Rank Information",
            value =f"**Tier:** {tier}\n"
    ...
    async def channel (self ,ctx ,channel :discord .TextChannel ):
        try :
            ...
            embed =discord .Embed (
            title =f"{_e('zcast')} Level-Up Channel Set",
    ...
    @level.command(name="status", description="Show current leveling system status and settings")
    async def level_status(self, ctx):
        """Display a summary of current leveling settings for this server."""
        try:
            settings = await self.get_guild_settings(ctx.guild.id)
            enabled     = settings.get('enabled', False)
            channel_id  = settings.get('channel_id')
            xp_per_msg  = settings.get('xp_per_message', 20)
            level_msg   = settings.get('level_message', 'Congratulations {user}! You have reached level {level}!')
            color       = settings.get('embed_color', '#000000')
            thumbnail   = settings.get('thumbnail_enabled', True)
            level_img   = settings.get('level_image')
            cooldown    = settings.get('cooldown_seconds', 60)

            channel = ctx.guild.get_channel(channel_id) if channel_id else None

            embed = discord.Embed(
                title=f"{_e('zpoll')} Leveling System Status",
                color=0xFF0000,
                timestamp=datetime.now(timezone.utc)
            )
            embed.add_field(
                name=f"{_e('zsettings')} System",
                value=(
                    f"**Status:** {'enabled' if enabled else 'disabled'}\n"
                    f"**XP per Message:** {xp_per_msg}\n"
            ...
            embed.add_field(
                name=f"{_e('zcast')} Notifications",
                value=(
                    f"**Level-Up Channel:** {channel.mention if channel else 'Current channel'}\n"
                    f"**Thumbnail:** {'yes' if thumbnail else 'no'}\n"
                    f"**Level-Up Image:** {_e('ztick')+' Set' if level_img else _e('zcross')+' Not set'}"
                ),
                inline=True
            )
            embed.add_field(
                name=f"{_e('zmsg')} Level-Up Message",
                value=f"```{level_msg[:200]}{'...' if len(level_msg) > 200 else ''}```",
                inline=False
            )
            embed.set_footer(text=f"Use `level enable` / `level disable` to toggle the system.")
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error in level status: {e}")

    # ─── level image ─────────────────────────────────────────────────────────

    @level.command(name="image", description="Set the level-up embed image URL")
    @commands.has_permissions(administrator=True)
    async def level_image_cmd(self, ctx, url: str):
        """Set a custom image shown in level-up embeds."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute("SELECT guild_id FROM leveling_settings WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                    exists = await cursor.fetchone()
                if exists:
                    await db.execute("UPDATE leveling_settings SET level_image = ? WHERE guild_id = ?", (url, ctx.guild.id))
                else:
                    await db.execute("INSERT INTO leveling_settings (guild_id, level_image) VALUES (?, ?)", (ctx.guild.id, url))
                await db.commit()
            embed = discord.Embed(
                title=f"{_e('ztick')} Level-Up Image Set",
                description=f"Level-up embeds will now show: [View Image]({url})",
                color=0x00FF00,
                timestamp=datetime.now(timezone.utc)
            )
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error setting level image: {e}")

    @level.command(name="clearimage", description="Remove the level-up embed image")
    @commands.has_permissions(administrator=True)
    async def level_clearimage(self, ctx):
        """Remove the custom level-up image."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute("UPDATE leveling_settings SET level_image = NULL WHERE guild_id = ?", (ctx.guild.id,))
                await db.commit()
            embed = discord.Embed(
                title=f"{_e('ztick')} Level-Up Image Cleared",
                description="The custom level-up image has been removed.",
                color=0x00FF00,
                timestamp=datetime.now(timezone.utc)
            )
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error clearing level image: {e}")

    # ─── level xprange ───────────────────────────────────────────────────────

    @level.command(name="xprange", description="Set the min and max XP awarded per message")
    @commands.has_permissions(administrator=True)
    async def level_xprange(self, ctx, min_xp: int, max_xp: int):
        """Set the XP range awarded per message (random between min and max)."""
        try:
            if min_xp < 1 or max_xp < 1:
                await ctx.send(f"{_e('zcross')} XP values must be at least 1.")
                return
            if min_xp > max_xp:
                await ctx.send(f"{_e('zcross')} Minimum XP must be ≤ maximum XP.")
                return
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute("SELECT guild_id FROM leveling_settings WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                    exists = await cursor.fetchone()
                if exists:
                    await db.execute(
                        "UPDATE leveling_settings SET min_xp = ?, max_xp = ?, xp_per_message = ? WHERE guild_id = ?",
                        (min_xp, max_xp, (min_xp + max_xp) // 2, ctx.guild.id)
                    )
                else:
                    await db.execute(
                        "INSERT INTO leveling_settings (guild_id, min_xp, max_xp, xp_per_message) VALUES (?, ?, ?, ?)",
                        (ctx.guild.id, min_xp, max_xp, (min_xp + max_xp) // 2)
                    )
                await db.commit()
            embed = discord.Embed(
                title=f"{_e('ztick')} XP Range Updated",
                description=f"Users will now earn between **{min_xp}** and **{max_xp}** XP per message.",
                color=0x00FF00,
                timestamp=datetime.now(timezone.utc)
            )
            await ctx.send(embed=embed)
    ...
    @level.command(name="preview", description="Preview how the level-up message will look")
    async def level_preview(self, ctx, level_number: int = 5):
        """Preview the level-up embed for a given level."""
        try:
            settings = await self.get_guild_settings(ctx.guild.id)
            level_message = settings.get('level_message', 'Congratulations {user}! You have reached level {level}!')
            level_message = level_message.replace('{user}', ctx.author.mention)
            level_message = level_message.replace('{level}', str(level_number))
            level_message = level_message.replace('{username}', ctx.author.display_name)
            level_message = level_message.replace('{server}', ctx.guild.name)
            ...
            embed = discord.Embed(
                title=f"{_e('ztada')} Level Up! *(Preview)*",
                description=level_message,
                color=embed_color,
                timestamp=datetime.now(timezone.utc)
            )
            ...
            if settings.get('level_image'):
                embed.set_image(url=settings['level_image'])
            embed.set_footer(text=f"Level {level_number} • {ctx.guild.name} • This is a preview")
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error in level preview: {e}")

    # ─── level reset group ───────────────────────────────────────────────────

    @level.group(name="reset", invoke_without_command=True, description="Reset leveling data")
    @commands.has_permissions(administrator=True)
    async def level_reset(self, ctx):
        """Level reset management — use `reset user` or `reset all`."""
        if ctx.invoked_subcommand is None:
            await ctx.send_help(ctx.command)

    @level_reset.command(name="user", description="Reset a specific user's XP and level")
    @commands.has_permissions(administrator=True)
    async def level_reset_user(self, ctx, member: discord.Member):
        """Reset a user's XP back to zero."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                ...
                await db.commit()
            embed = discord.Embed(
                title=f"{_e('ztick')} User XP Reset",
                description=f"Successfully reset all XP and level data for {member.mention}.",
                color=0x00FF00,
                timestamp=datetime.now(timezone.utc)
            )
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error resetting user XP: {e}")

    @level_reset.command(name="all", description="Reset ALL users' XP in this server")
    @commands.has_permissions(administrator=True)
    async def level_reset_all(self, ctx):
        """Reset every user's XP in this server. This cannot be undone."""
        try:
            confirm_embed = discord.Embed(
                title=f"{_e('zwarning')} Confirm Reset",
                description=(
                    "Are you sure you want to **reset ALL users' XP** in this server?\n\n"
                    "This **cannot be undone**. Reply with `confirm` within 30 seconds to proceed."
                ),
                color=0xFF0000,
            )
            ...
            try:
                ...
                await ctx.send(f"{_e('zcross')} Reset cancelled — timed out.")
                return

            async with aiosqlite.connect(self.db_path) as db:
                await db.execute("DELETE FROM user_xp WHERE guild_id = ?", (ctx.guild.id,))
                await db.execute("DELETE FROM users WHERE guild_id = ?", (ctx.guild.id,))
                await db.commit()

            embed = discord.Embed(
                title=f"{_e('ztick')} All XP Reset",
                description="All user XP and level data for this server has been cleared.",
                color=0x00FF00,
                timestamp=datetime.now(timezone.utc)
            )
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error resetting all XP: {e}")

    # ─── level unblacklist group ─────────────────────────────────────────────

    @level.group(name="unblacklist", invoke_without_command=True, description="Remove from the leveling blacklist")
    @commands.has_permissions(administrator=True)
    async def level_unblacklist(self, ctx):
        """Remove channels or roles from the leveling blacklist."""
        if ctx.invoked_subcommand is None:
            await ctx.send_help(ctx.command)

    @level_unblacklist.command(name="channel", description="Un-blacklist a channel")
    @commands.has_permissions(administrator=True)
    async def unblacklist_channel(self, ctx, channel: discord.TextChannel):
        """Remove a channel from the leveling blacklist."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    "DELETE FROM leveling_blacklist WHERE guild_id = ? AND target_id = ? AND target_type = 'channel'",
                    (ctx.guild.id, channel.id)
                )
                await db.commit()
            await ctx.send(f"{_e('ztick')} {channel.mention} has been removed from the leveling blacklist.")
        except Exception as e:
            logger.error(f"Error unblacklisting channel: {e}")

    @level_unblacklist.command(name="role", description="Un-blacklist a role")
    @commands.has_permissions(administrator=True)
    async def unblacklist_role(self, ctx, role: discord.Role):
        """Remove a role from the leveling blacklist."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    "DELETE FROM leveling_blacklist WHERE guild_id = ? AND target_id = ? AND target_type = 'role'",
                    (ctx.guild.id, role.id)
                )
                await db.commit()
            await ctx.send(f"{_e('ztick')} {role.mention} has been removed from the leveling blacklist.")
        except Exception as e:
            logger.error(f"Error unblacklisting role: {e}")

    # ─── level blacklist channel / role shortcuts ─────────────────────────────

    @blacklist.command(name="channel", description="Blacklist a channel from leveling")
    @commands.has_permissions(administrator=True)
    async def blacklist_channel(self, ctx, channel: discord.TextChannel):
        """Blacklist a channel so messages in it don't earn XP."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    "INSERT OR REPLACE INTO leveling_blacklist (guild_id, target_id, target_type) VALUES (?, ?, 'channel')",
                    (ctx.guild.id, channel.id)
                )
                await db.commit()
            await ctx.send(f"{_e('ztick')} {channel.mention} has been added to the leveling blacklist.")
        except Exception as e:
            logger.error(f"Error blacklisting channel: {e}")

    @blacklist.command(name="role", description="Blacklist a role from leveling")
    @commands.has_permissions(administrator=True)
    async def blacklist_role(self, ctx, role: discord.Role):
        """Blacklist members with this role so they don't earn XP."""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    "INSERT OR REPLACE INTO leveling_blacklist (guild_id, target_id, target_type) VALUES (?, ?, 'role')",
                    (ctx.guild.id, role.id)
                )
                await db.commit()
            await ctx.send(f"{_e('ztick')} {role.mention} has been added to the leveling blacklist.")
        except Exception as e:
            logger.error(f"Error blacklisting role: {e}")

    # ─── addreward / removereward aliases ─────────────────────────────────────

    @level.command(name="addreward", description="Add a level reward role (shortcut)")
    @commands.has_permissions(administrator=True)
    async def addreward(self, ctx, level_num: int, role: discord.Role, remove_previous: bool = False):
        """Shortcut for `level rewards add`. Add a role reward for reaching a level."""
        await ctx.invoke(self.rewards_add, level=level_num, role=role, remove_previous=remove_previous)

    @level.command(name="removereward", description="Remove a level reward role (shortcut)")
    @commands.has_permissions(administrator=True)
    async def removereward(self, ctx, level_num: int):
        """Shortcut for `level rewards remove`. Remove the reward for a level."""
        await ctx.invoke(self.rewards_remove, level=level_num)

...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    await message.edit(content=f"🎉 **GIVEAWAY ENDED** 🎉", embed=embed)
    ...
    async def gstart(self, ctx,
        ...
        if winners >=  15:
            embed = discord.Embed(title=f"{_e('zwarning')} Access Denied",
        ...
        if converted == -1:
            embed = discord.Embed(title=f"{_e('zcross')} Error",
        ...
        embed = discord.Embed(title=f"🎉 {prize}",
                              description=f"➡️ Winner(s): **{winners}**\n➡️ Hosted by {ctx.author.mention}\n➡️ Ends <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\n➡️ React with 🎉 to participate!", color=0xFF0000)
    ...
    async def gend(self, ctx, message_id = None):
        ...
        if message_id is not None:
            ...
            if len(users) < 1:
                await ctx.send(f"{_e('ztick')} Successfully Ended the giveaway in <#{int(re[6])}>")

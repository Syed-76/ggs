...
class Welcomer(commands.Cog):
    ...
    async def simple_setup(self, ctx):
        ...
        placeholders = {
            ...
            "user_avatar": ctx.author.display_avatar.url if ctx.author.avatar else ctx.author.default_avatar.url,
    ...
    async def greet_channel(self, ctx):
        ...
        def generate_view(page):
            select_menu = Select(
                ...
                options=[
                    discord.SelectOption(label=channel.name, emoji="#️⃣", value=str(channel.id))
    ...
    async def greet_autodelete(self, ctx, time: str):
        ...
        await ctx.send(f"{_e('ztick')} Auto delete duration has been set to **{auto_delete_duration}** seconds.")

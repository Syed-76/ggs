...
class Kick(commands.Cog):
    ...
    async def kick_command(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        member_avatar_url = member.display_avatar.url if member.avatar else None

        embed = discord.Embed(
            description=(
                f"**{_e('ztick')} | {member.mention} has been kicked successfully\nReason:{reason}**"
            ),
            color=self.color # Uses the red color 0xFF0000
        )
        embed.set_author(name=f"Successfully Kicked {member.name}")
        embed.set_footer(text=f"Action by {ctx.author.name}", icon_url=ctx.author.display_avatar.url if ctx.author.avatar else None)
